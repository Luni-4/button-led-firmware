...
/// A `light` device.
///
/// An initial placeholder for constructing a [`CompleteLight`].
...
/// A `light` device containing only the route to turn a light on.
///
    ...
    S: ValueFromRef + Send + Sync + 'static,
{
    ...
    /// Adds a [`Route`] with a stateless handler to [`Light`].
    #[must_use]
    ...
    /// Adds a [`Route`] with a stateful handler to [`Light`].
    #[must_use]

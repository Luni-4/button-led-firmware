...
// Delay in milliseconds to wait after an asynchronous operation
    ...
    S: ValueFromRef + Send + Sync + 'static,
{
    ...
    /// Runs the [`Server`].
    pub async fn run(self, stack: Stack<'static>) -> Result<(), Error<TcpError>> {
        let ip = Self::get_ip(stack).await;
        ...
        let acceptor = tcp.bind(socket).await.unwrap();

        let mut server = EdgeServer::<SERVER_SOCKETS, RX_SIZE, MAXIMUM_HEADERS_COUNT>::new();

        server.run(Some(TIMEOUT), acceptor, self.handler).await
    }
